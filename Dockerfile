...
RUN apt-get update -y && apt-get upgrade -y && \
	echo "deb http://archive.debian.org/debian/ stretch contrib main non-free" > /etc/apt/sources.list.d/stretch.list && \
 	apt-get update -y && \
	apt-get install python2.7 curl wget git mercurial -y && \
